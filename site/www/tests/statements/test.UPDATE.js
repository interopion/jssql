(function() {
	...
	QUnit.asyncTest("UPDATE t1 SET val = 'updated' WHERE id = 1", function(assert) {
		jsSQL(function(API) {
			API.query("UPDATE t1 SET val = 'updated' WHERE id = 1", function(err, result) {
				var table = API.getTable("t1");
				assert.ok(!err, err);
				assert.deepEqual(
					table.rows['1'].toJSON(), 
	...
	QUnit.asyncTest("UPDATE t1 SET val = id + '-updated'", function(assert) {
		jsSQL(function(API) {
			API.query("UPDATE t1 SET val = id + '-updated'", function(err, result) {
				var table = API.getTable("t1");
				assert.ok(!err, err);
				assert.deepEqual(
					table.rows['1'].toJSON(), 
	...
	QUnit.asyncTest("UPDATE t1 SET val = id + '-updated' WHERE id = 1", function(assert) {
		jsSQL(function(API) {
			API.query("UPDATE t1 SET val = id + '-updated' WHERE id = 1", function(err, result) {
				var table = API.getTable("t1");
				assert.ok(!err, err);
				assert.deepEqual(
					table.rows['1'].toJSON(), 
	...
	QUnit.asyncTest("Make sure that the primary index is re-ordered after update", function(assert) {
		jsSQL(function(API) {
			...
			API.query("UPDATE t1 SET val = 'd' WHERE id = 2", function(err, result) {
	...
	QUnit.asyncTest('Make sure that "beforeupdate" and "update" events are triggered', function(assert) {
		jsSQL(function(API) {
			var eventsFired = {
				...
				table = API.getTable("t1"),
				row   = table.rows['2'],
				sql   = "UPDATE t1 SET val = 'd' WHERE id = 2;";

			var h1 = JSDB.events.on("beforeupdate:table", function(e, t) {
				eventsFired[e]++;
				assert.strictEqual(t, table, "The Table instance gets passed to the event listeners of " + e + " event");
			});

			var h2 = JSDB.events.on("beforeupdate:row", function(e, r) {
				eventsFired[e]++;
				assert.strictEqual(r, row, "The TableRow instance gets passed to the event listeners of " + e + " event");
			});

			var h3 = JSDB.events.on("update:table", function(e, t) {
				eventsFired[e]++;
				assert.strictEqual(t, table, "The Table instance gets passed to the event listeners of " + e + " event");
			});

			var h4 = JSDB.events.on("update:row", function(e, r) {
				eventsFired[e]++;
				assert.strictEqual(r, row, "The TableRow instance gets passed to the event listeners of " + e + " event");
			});
			...
			API.query(sql, function(err, result) {
				...
				JSDB.events.off("beforeupdate:table", h1);
				JSDB.events.off("beforeupdate:row", h2);
				JSDB.events.off("update:table", h3);
				JSDB.events.off("update:row", h4);
				QUnit.start();
			});
		});
	});
	...
	QUnit.asyncTest("Make sure that an unique column cannot be updated to existing value", function(assert) {
		jsSQL(function(API) {
			var table = API.getTable("t1");
			API.query("UPDATE t1 SET id = 2 WHERE id = 1", function(err, result) {
	...
	QUnit.asyncTest('Make sure that "beforeupdate:table" and "beforeupdate:row" events are cancelable', function(assert) {
		jsSQL(function(API) {
			...
			API.query("UPDATE t1 SET val = 'd'", function(err) {
				assert.ok(!err, "Must not fail: " + err);
				assert.deepEqual(table.rows['1'].toJSON(), { id : 1, val : 'a' }, "The first row IS NOT updated");
				assert.deepEqual(table.rows['2'].toJSON(), { id : 2, val : 'b' }, "The second row IS NOT updated");
				assert.deepEqual(table.rows['3'].toJSON(), { id : 3, val : 'c' }, "The third row IS NOT updated");

				JSDB.events.one("beforeupdate:row", false);

				API.query("UPDATE t1 SET val = 'e'", function(err, result, idx) {
					assert.ok(!err, "Must not fail: " + err);
					assert.deepEqual(table.rows['1'].toJSON(), { id : 1, val : 'a' }, "The first row IS NOT updated");
					assert.deepEqual(table.rows['2'].toJSON(), { id : 2, val : 'e' }, "The second row is updated");
					assert.deepEqual(table.rows['3'].toJSON(), { id : 3, val : 'e' }, "The third row is updated");
					QUnit.start();
				});
			});
		});
